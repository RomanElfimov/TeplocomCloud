...
protocol SensorsExpertSettingsDisplayLogic: AnyObject {
    func displayData(event: SensorsExpertSettings.Model.ViewModel.ViewModelData)
}

// MARK: - View Controller Class

final class SensorsExpertSettingsViewController: UIViewController {

    private(set) var router: (SensorsExpertSettingsRoutingLogic & SensorsExpertSettingsDataPassingProtocol)?

    // MARK: - Private Properties

    private var interactor: (SensorsExpertSettingsBusinessLogic & SensorsExpertSettingsStoreProtocol)?
    ...
    private let intergrationWindowDataSourceArray = (1...60).map { "\($0) мин." }
    private let failureValueDataSourceArray = (1...99).map { "\($0) ˚C" }
    private let notificationIntervalDataSourceArray = (1...60).map { "\($0) мин." }
    private let repeatNotificationsDataSourceArray = (1...100).map { "\($0)" }
    ...
    private func setup() {
        ...
        let interactor = SensorsExpertSettingsInteractor()
        let presenter = SensorsExpertSettingsPresenter()
        let router = SensorsExpertSettingsRouter()
    ...
    override func viewDidLoad() {
        ...
        interactor?.makeRequest(event: .fetchSensorsExpertSettings)
    ...
    private func configureUI(with data: TemperatureSensorExpertSettingsViewModel) {

        intergrationWindowTextField.text = data.roundWindow
        failureValueTextField.text = data.defaultValue

        minNotificationSwitch.isOn = data.notifyOnLowerLimit
        maxNotificationSwitch.isOn = data.notifyOnUpperLimit
        failureNotificationSwitch.isOn = data.notifyOnFailure

        notificationIntervalTextField.text = data.notifyInterval
        repeatNotificationsCountTextField.text = data.notifyQuantity
    }

    // MARK: - Selectors

    @objc func cancelButtonTapped() {
        router?.navigateToSensorSettings()
    }

    @objc func saveButtonTapped() {

        // Окно интеграции
        guard let intergrationWindow = intergrationWindowTextField.text, intergrationWindow != "" else { return }
        let roundWindow = (Int(intergrationWindow.replacingOccurrences(of: " мин.", with: "")) ?? 0) * 60

        // Установить значение при аварии
        guard let failureValue = failureValueTextField.text, failureValue != "" else { return }
        let defaultValue = (Int(failureValue.replacingOccurrences(of: " ˚C", with: ""))  ?? 0) * 100

        // Оповещение при достижении минимума
        let notifyOnLowerLimit = minNotificationSwitch.isOn

        // Оповещение при достижении максимума
        let notifyOnUpperLimit = maxNotificationSwitch.isOn

        // Оповещение при аварии
        let notifyOnFailure = failureNotificationSwitch.isOn

        // Интервал оповещения
        guard let notificationInterval = notificationIntervalTextField.text, notificationInterval != "" else { return }
        let notifyInterval = (Int(notificationInterval.replacingOccurrences(of: " мин.", with: "")) ?? 0) * 60

        // Количество повторных оповещений
        guard let notifyQuantity = repeatNotificationsCountTextField.text, notifyQuantity != "" else { return }

        let editExpertSettingsParameters: [String: Any] = ["roundWindow": roundWindow,
                                                           "defaultValue": defaultValue,
                                                           "notifyOnLowerLimit": notifyOnLowerLimit,
                                                           "notifyOnUpperLimit": notifyOnUpperLimit,
                                                           "notifyOnFailure": notifyOnFailure,
                                                           "notifyInterval": notifyInterval,
                                                           "notifyQuantity": notifyQuantity]

        interactor?.makeRequest(event: .editSensorsExpertSettings(parameters: editExpertSettingsParameters))
    }
...
extension SensorsExpertSettingsViewController: SensorsExpertSettingsDisplayLogic {
    func displayData(event: SensorsExpertSettings.Model.ViewModel.ViewModelData) {
        switch event {

        case .fetchSensorsExpertSettings(let data):
            configureUI(with: data)

        case .editSensorsExpertSettingseditSensorsExpertSettings(let statusCode):

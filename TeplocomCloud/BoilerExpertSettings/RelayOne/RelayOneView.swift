...
protocol RelayOneDisplayLogic: AnyObject {
    func displayData(event: RelayOne.Model.ViewModel.ViewModelData)
}

// MARK: - View class

final class RelayOneView: UIView {

    // MARK: - Private Properties

    private var interactor: RelayOneBusinessLogic?
    ...
    private lazy var titleLabel: UILabel = {
        let label = UILabel()
        label.text = "Управление котлом через релейный выход №1"
    ...
    private lazy var relayImageView: UIImageView = {
        ...
        iv.image = UIImage(named: "Relay1Image")
        return iv
    }()
    ...
    private func setupCleanSwift() {
        let view = self
        let interactor = RelayOneInteractor()
        let presenter = RelayOnePresenter()
    ...
    @objc func saveButtonTapped() {
        let parameters: [String: Any] = ["boilerControlProtocol": "relay1",
...
extension RelayOneView: RelayOneDisplayLogic {
    func displayData(event: RelayOne.Model.ViewModel.ViewModelData) {
        switch event {
        case .fetchExpertSettings(data: let data):

            if data.boilerControlProtocol == "relay1" {
                setupUI(with: data)
            }
...
extension RelayOneView: UITableViewDelegate, UITableViewDataSource {
